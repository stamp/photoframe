...
app.post("/loadFromSearch", async (req, res) => {
  ...
  const data = await libraryApiSearch(req, parameters);
...
app.get("/getQueue", async (req, res) => {
  ...
  if (cachedPhotos) {
    ...
    try {
      const data = await libraryApiSearch(req, stored.parameters);
...
async function libraryApiSearch(req, parameters) {
  console.log("libraryApiSearch");

  parameters.pageSize = config.searchPageSize;

  var makeApiRequest = async function(retried, authToken, refreshToken) {
    let photos = [];
    let nextPageToken = null;
    let error = null;

    console.log("makeApiRequest", retried);
    try {
      ...
      console.log("request error", err.status, retried);

      const r = await new Promise((resolve, reject) => {
        refresh.requestNewAccessToken("google", refreshToken, async function(
          err,
          accessToken2,
          refreshToken2
        ) {
          console.log("refreshing token", err, accessToken2, refreshToken2);
          if (err || !accessToken2) {
  ...
  return await makeApiRequest(false, req.user.token, req.user.refreshToken);
}
